...
def drawlossplot( m,loss_g,loss_d,e):
    ...
    plt.legend()
    plt.savefig("celeba_dcgan_loss_epoch%d" %e)
    plt.close()
...
def generateimage(m,gd,e,new=False):
       if new :
        ...
        g = m.g(g_gs)
       g = g.data.cpu().numpy()
       g = g.reshape(m.batch_size,3,y_size,x_size)
       #print(g)
       ...
       for i in range(m.batch_size):
        ...
        ax.set_axis_off()
        tp = np.moveaxis(g[i],0,-1)
        #print(tp.shape)
        plt.imshow(tp ,shape=(64,64),cmap='Greys_r')
